...
发布用定稿（issue #6）。三块内容：用户复制的提示词、配图分步文案、必须保留的警告。发布前唯一要核对的事：仓库已合入 `plugin/skills/server-install/SKILL.md`，提示词里的 raw 链接能打开。二、三节的分步文案与该 SKILL.md 的 Step 4 是同一流程的两份呈现——改任一处要同步另一处（ADR-0005）。
...
1. 搜「ed api」，进 edstem.org 的 API Tokens 设置页（悉尼大学可直达 `https://edstem.org/au/settings/api-tokens`）。
...
- 对 AI 说「我的 Canvas token 过期了」，它会带你走一遍刷新，不用重装。
