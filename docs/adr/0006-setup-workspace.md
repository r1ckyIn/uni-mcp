...
- **只服务新工作区，不迁移旧的**（2026-08-14 Ricky 定，见 design-decisions 工作区一节）。Step 1 的「半成品工作区」分支是为「上次跑到一半」设计的，套在手工建过的工作区上会误判：状态文件不在 `status/` 就会被补出第二个状态源，手写的 AGENTS.md 会被模板刷掉。这条边界不写进 SKILL.md——插件面向的新用户遇不到，写进去反而是常驻上下文成本；它约束的是维护者，别再为「原地升级老工作区」开票。
