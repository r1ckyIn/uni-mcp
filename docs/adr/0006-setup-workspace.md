...
3. **状态真相源固定四个文件，文件名英文、内容跟随用户语言。** `status/{assessments,todos,weekly,not-doing}.md`。`assessments.md` 按 due 时间倒排（outline 给不出日期的行——"Multiple weeks"、"Formal exam period"——保留原词、排在有日期的行之后），行的粒度是 outline 的考核项而不是 Canvas 作业对象（一个 10% 的周测在 Canvas 里是十几个对象，照抄会让五门课变成六十行）；备注列同时承担溯源与作废账本（值变了保留旧值和变更日期，不删）。重跑是合并刷新、不是重新生成——重新生成会把作废账本连同人写的行一起抹掉。状态列在 setup 阶段留空：摸底读的是 deadline 不是提交状态，`canvas_get_all_grades` 只给课级总分、给不出逐项状态，「交没交、多少分」是日后按课现问的活问题。另外三个是用户的：首次建好标题就不再动，重跑也不动。「不许另开状态文件」写进 AGENTS.md，靠后续会话守。
