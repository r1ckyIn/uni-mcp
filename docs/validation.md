...
| 5 | Codex-side manifests: `.agents/plugins/marketplace.json` structure, `.codex-plugin/plugin.json` under each entry's `source` dir, name/version sync with the Claude manifest | `node scripts/check-codex-manifests.mjs` | — |
...
Check 5 has no host CLI to lean on: `codex plugin marketplace add` / `codex plugin add` validate nothing (codex-cli 0.144.1, tested 2026-08-13 — a manifest with no `version` installs silently), so the script is a static mirror of the codex ingestion schema. The schema authority is the plugin-creator system skill shipped inside codex-cli (`references/plugin-json-spec.md` + `scripts/validate_plugin.py`); re-check it against the script when bumping the codex CLI. Like check 2, the plugin manifest path is derived from the marketplace entry's `source`, so a dangling source turns red. See ADR-0003 for the layout and field decisions.
